...
#region MEMBER_FIELDS
        ...
        /// TANGO_SUPPORT_UNITY_DLL_C  2017.09.25 Minji Kim
        /// </summary>
        internal const string TANGO_SUPPORT_UNITY_DLL = "tango_support_api";
        // internal const string TANGO_SUPPORT_UNITY_DLL_C = "tango_support_api 1";
...
#region DEPTH_FUNCTIONS
        ...
        /// <summary>
        /// Minji Kim 2017.09.26
        /// Find edges near the selected point
        /// C wrapper function
        /// </summary>
        /// <param name="pointCloud"></param>
        /// <param name="colorCameraTimestamp"></param>
        /// <param name="uvCoordinates"></param>
        /// <returns></returns>
        public static bool FindEdgesNearPoint(TangoUnityImageData imageBuffer,
            TangoPointCloudData pointCloud,
            double colorCameraTimestamp,
            Vector2 uvCoordinates,
            //out Vector3[][] end_points,
            //out Vector3[] nearest_point,
            out TangoSupportEdge[] edges,
            out int num_edges)
        {
            if (pointCloud == null) Debug.LogError("FindEdgesNearPoint::point cloud null");
            TangoPoseData depth_T_colorCameraPose = new TangoPoseData();
            ...
            IntPtr uEdges = IntPtr.Zero;
            
            returnValue = TangoSupportAPI.TangoSupport_findEdgesNearPoint(
                ref tangoPointCloudIntPtr,
                ref pointCloudTranslation,
                ref pointCloudRotation,
                ref buffer,
            ...
            if (returnValue != Common.ErrorType.TANGO_SUCCESS)
            {
                Debug.LogError("TangoSupport_findEdgesNearPoint error. " + Environment.StackTrace);
                return false;
            }
            else
            {
                TangoSupportEdge n_edges = new TangoSupportEdge();
                int edgeSize = Marshal.SizeOf(n_edges.GetType());
                Debug.Log("<<<<<<<<<< Edges number = " + num_edges);
                edges = new TangoSupportEdge[num_edges];
                
                for(int i = 0; i < num_edges; i++)
                {
                    try
                    {
                        var deref2 = (TangoSupportEdge)Marshal.PtrToStructure(new IntPtr(uEdges.ToInt32()), 
                            typeof(TangoSupportEdge));
                        /*
                        Debug.Log("<<<<<<<<<< Start point" + i + " : x = " + deref2.end_points_x1 +
                        ", y = " + deref2.end_points_y1 + ", z = " + deref2.end_points_z1);
                        Debug.Log("<<<<<<<<<< End point" + i + " : x = " + deref2.end_points_x2 +
                        ", y = " + deref2.end_points_y2 + ", z = " + deref2.end_points_z2);
                        Debug.Log("<<<<<<<<<< Nearest Point on the edge" + i + " : " + deref2.closest_point_on_edge_x);
                        */
                        edges[i] = deref2;
                    }
                    catch(Exception e)
...
#region DATA_TYPES
        ...
        public struct TangoErrorType
        {
            ...
            public const int TANGO_NO_MOTION_TRACKING_PERMISSION = -3;
            /// The input argument is invalid.
            public const int TANGO_INVALID = -2;
            /// This error code denotes some sort of hard error occurred.
            public const int TANGO_ERROR = -1;
            /// This code indicates success.
            public const int TANGO_SUCCESS = 0;
        }

        /// <summary>
        /// Minji Kim 2017.09.25
        /// Tango C Edge Struct Wrapper
        /// </summary>
        /// 
        /*
        [StructLayout(LayoutKind.Sequential)]
        public struct TangoSupportEdge
        {
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 6)]
            public float[, ] end_points;

            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            public float[] closest_point_on_edge;
        }
        */
        [StructLayout(LayoutKind.Sequential)]
        public struct TangoSupportEdge
        {
            ...
            public float end_points_x1;
            public float end_points_y1;
            public float end_points_z1;
            public float end_points_x2;
            public float end_points_y2;
            public float end_points_z2;

            [MarshalAs(UnmanagedType.R4)]
            public float closest_point_on_edge_x;
            public float closest_point_on_edge_y;
            public float closest_point_on_edge_z;
        }

        /// <summary>
        /// Minji Kim 2017.09.25
        /// Tango C Rotation Struct Wrapper
        /// </summary>
        [StructLayout(LayoutKind.Sequential)]
        public struct TangoSupportRotation
        {
            /// Not apply any rotation.
            const int ROTATION_IGNORED = -1;
        ...
        private struct TangoSupportAPI
        {
            ...
            ///
            /// 2017.10.01 Minji Kim
            /// fixed TangoSupportEdge type as IntPtr
            ///
            [DllImport(TANGO_SUPPORT_UNITY_DLL)]
            public static extern int TangoSupport_findEdgesNearPoint(
                ref TangoPointCloudIntPtr point_cloud,
                ref DVector3 poitnCloudTranslation,
                ref DVector4 pointCloudOrientation,
                ref TangoImageBuffer image_buffer,
                ref Vector2 uvCoordinatesInColorCamera,
                OrientationManager.Rotation display_rotation,
                ref DVector3 colorCameraTranslation,
                ref DVector4 colorCameraOrientation,
                out IntPtr edges,
                ref int number_of_edges);
            ...
            public static int TangoSupport_findEdgesNearPoint(
                ref TangoPointCloudIntPtr point_cloud,
                ref DVector3 poitnCloudTranslation,
                ref DVector4 pointCloudOrientation,
                ref TangoImageBuffer image_buffer,
            ...
            {
                number_of_edges = -1;
                TangoSupportEdge tEdges = new TangoSupportEdge();
                edges = Marshal.AllocHGlobal(Marshal.SizeOf(tEdges));
                Marshal.StructureToPtr(tEdges, edges, false);
                return Common.ErrorType.TANGO_SUCCESS;
            }
